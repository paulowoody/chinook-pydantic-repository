...
def test_album_repository_get_all(mock_cursor):
    ...
    mock_cursor.execute.assert_called_once_with(
        "SELECT * FROM album ORDER BY album_id LIMIT %s OFFSET %s;", 
        (10, 0)
    )
...
def test_album_repository_get_by_artist_id(mock_cursor):
    ...
    mock_cursor.execute.assert_called_once_with(
        "SELECT * FROM album WHERE artist_id = %s ORDER BY title;", 
        (1,)
    )
...
def test_playlist_track_repository_get_by_composite_id(mock_cursor):
    ...
    mock_cursor.execute.assert_called_once_with(
        "SELECT * FROM playlist_track WHERE playlist_id = %s AND track_id = %s;", 
        (1, 5)
    )
